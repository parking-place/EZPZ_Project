...
if __name__ == '__main__': #현재 파일에서 실행될때만 실행됨
    ...
    daum_news = daum_news_crawler.get_news('삼성전자') #다음뉴스 크롤러 실행 확인
    naver_news = naver_news_crawler.get_news('삼성전자') #네이버 뉴스크롤러 실행이 안되니 천천히 해보기
    all_news = pd.concat([daum_news, naver_news], ignore_index=True)
    

    #csv저장로직------------------------
    #os.makedirs(dir_path) # 디렉토리 생성
    all_news.to_csv(os.path.join(dir_path, f'\\{keyword}_daum_naver.csv'), index=False, encoding='utf-8')
    #-----------------------------------
